...
export class TimeTrackingService {
  ...
  async importTimesheets(fileBuffer: Buffer, businessId: string, user: any): Promise<any> {
    ...
    for (const row of data) {
      try {
        ...
        if (!email || !dateStr || !startStr) continue;
  ...
  async getEmployeeRecord(userId: string, businessId?: string, user?: any): Promise<string> {
    const where: any = { userId };
    if (businessId) {
      if (user) await this.validateBusinessAccess(businessId, user);
  ...
  async clockIn(
    ...
    if (!opts?.bypassGeofence) {
      if (!locationId) {
        throw new BadRequestException('Shift location is required for clock-in');
      }

      const hasCoords = typeof lat === 'number' && Number.isFinite(lat) && typeof lng === 'number' && Number.isFinite(lng);
      if (!hasCoords) {
        throw new BadRequestException('Location permission is required to clock in');
      }
      ...
      if (!location) throw new BadRequestException('Clock-in location not found');
      if (location.businessId !== employee.businessId) throw new BadRequestException('Invalid clock-in location');
      if (typeof location.geoLat !== 'number' || typeof location.geoLng !== 'number') {
        throw new BadRequestException('Clock-in geofence is not configured for this site');
      }
      ...
      if (distance > radiusMeters) {
        throw new BadRequestException(
          `You are about ${Math.round(distance)}m from ${location.name}. You must be within ${radiusMeters}m to clock in`,
        );
      }
    }
  ...
  private async validateWithinClockLocation(employeeId: string, locationId: string | undefined, lat?: number, lng?: number) {
    if (!locationId) {
      throw new BadRequestException('Shift location is required');
    }

    const hasCoords = typeof lat === 'number' && Number.isFinite(lat) && typeof lng === 'number' && Number.isFinite(lng);
    if (!hasCoords) {
      throw new BadRequestException('Location permission is required');
    }
    ...
    if (!employee) throw new BadRequestException('Employee not found');
    if (!location) throw new BadRequestException('Clock location not found');
    if (location.businessId !== employee.businessId) throw new BadRequestException('Invalid clock location');
    if (typeof location.geoLat !== 'number' || typeof location.geoLng !== 'number') {
      throw new BadRequestException('Clock-in geofence is not configured for this site');
    }
    ...
    if (distance > radiusMeters) {
      throw new BadRequestException(
        `You are about ${Math.round(distance)}m from ${location.name}. You must be within ${radiusMeters}m to continue`,
      );
    }
  }

  private async getClockLocationStatus(employeeId: string, locationId: string | undefined, lat?: number, lng?: number) {
    if (!locationId) {
      throw new BadRequestException('Shift location is required');
    }
  ...
  async locationPing(employeeId: string, lat?: number, lng?: number) {
    const activeTimesheet = await this.prisma.timesheet.findFirst({
      where: { employeeId, endTime: null },
      include: { breaks: { where: { endTime: null } }, location: true, employee: true } as any,
      orderBy: { startTime: 'desc' },
    });

    if (!activeTimesheet) {
      throw new BadRequestException('No active timesheet found');
    }
    ...
    const onBreak = Array.isArray((activeTimesheet as any).breaks) && (activeTimesheet as any).breaks.length > 0;
    if (onBreak) {
    ...
    if (!status.within) {
      ...
      const siteName = status.location?.name || 'site';
      const siteAddress = status.location?.address || '';
  ...
  private async notifyLeavingSite(args: {
    employeeId: string;
    businessId: string;
    ...
    if (args.employeeUserId) recipientIds.add(args.employeeUserId);
    if (business?.ownerId) recipientIds.add(business.ownerId);
    for (const e of adminEmployees) if (e.userId) recipientIds.add(e.userId);
    for (const u of superAdmins) recipientIds.add(u.id);
  ...
  async requireClockInShift(employeeId: string) {
    const now = new Date();
    ...
    if (!shift) {
      throw new BadRequestException('No scheduled shift available for clock-in today');
    }
    if (!shift.locationId) {
      throw new BadRequestException('This shift has no assigned site. Contact your manager.');
    }
    return shift;
  }

  async adminClockIn(employeeId: string, locationId?: string, user?: any): Promise<any> {
    if (user) {
    ...
    if (!existingShift) {
      ...
      if (records && records.length > 0) {
        ...
        const overlapsTime = (rec: any, s: Date) => {
        ...
        if (blocked) {
          throw new BadRequestException('Employee is not available for this time');
        }
      }
  ...
  async adminClockOut(employeeId: string, user?: any): Promise<any> {
    if (user) {
        const employee = await this.prisma.employee.findUnique({ where: { id: employeeId } });
        if (!employee) throw new BadRequestException('Employee not found');
        await this.validateBusinessAccess(employee.businessId, user);
  ...
  async clockOut(employeeId: string, ip?: string, note?: string, lat?: number, lng?: number): Promise<Timesheet> {
    const activeTimesheet = await this.prisma.timesheet.findFirst({
      where: {
        employeeId,
        endTime: null,
    ...
    if (!activeTimesheet) {
      throw new BadRequestException('No active timesheet found to clock out from');
    }
  ...
  async startBreak(employeeId: string, type: string = 'UNPAID', lat?: number, lng?: number): Promise<any> {
    const activeTimesheet = await this.prisma.timesheet.findFirst({
      where: {
        employeeId,
        endTime: null,
    ...
    if (!activeTimesheet) {
      throw new BadRequestException('No active timesheet found to start a break');
    }
  ...
  async endBreak(employeeId: string, lat?: number, lng?: number): Promise<any> {
    const activeTimesheet = await this.prisma.timesheet.findFirst({
      where: {
        employeeId,
        endTime: null,
  ...
  async getEmployeeStatus(employeeId: string, user?: any) {
    if (user) {
        const employee = await this.prisma.employee.findUnique({ where: { id: employeeId } });
        if (employee) await this.validateBusinessAccess(employee.businessId, user);
    }
    const activeTimesheet = await this.prisma.timesheet.findFirst({
      where: {
        employeeId,
  ...
  async getTimesheets(employeeId: string, start: Date, end: Date, user?: any): Promise<Timesheet[]> {
      if (user) {
        const employee = await this.prisma.employee.findUnique({ where: { id: employeeId } });
        if (employee) await this.validateBusinessAccess(employee.businessId, user);
      }
  ...
  async updateTimesheet(id: string, data: any, user?: any): Promise<Timesheet> {
    ...
    const breaks: Array<{ startTime: string; endTime?: string; type?: string }> = Array.isArray(data.breaks) ? data.breaks : [];
    for (const b of breaks) {
      const bs = new Date(b.startTime);
      const be = b.endTime ? new Date(b.endTime) : undefined;
      if (Number.isNaN(bs.getTime()) || (be && Number.isNaN(be.getTime()))) {
        throw new BadRequestException('Invalid break time format');
      }
      if (be && bs > be) {
      ...
      if (start && end && be && (bs < start || be > end)) {
        ...
        throw new BadRequestException('Break must be within timesheet start and end time');
      }
    }
    ...
    let result: Timesheet | null = null;
    try {
      ...
      const message = e?.message || 'Failed to update timesheet';
  ...
  async deleteTimesheet(id: string, requestingEmployeeId?: string, user?: any): Promise<Timesheet> {
    const timesheet = await this.prisma.timesheet.findUnique({ where: { id }, include: { employee: true } });
    if (!timesheet) throw new BadRequestException('Timesheet not found');
  ...
  async restoreTimesheet(id: string, requestingEmployeeId?: string, user?: any): Promise<Timesheet> {
    const timesheet = await this.prisma.timesheet.findUnique({ where: { id }, include: { employee: true } });
    if (!timesheet) throw new BadRequestException('Timesheet not found');
    ...
    if (requestingEmployeeId && timesheet.employeeId !== requestingEmployeeId) {
        throw new ForbiddenException('You can only restore your own timesheets');
    }
  ...
  async getBusinessTimesheets(businessId: string, start: Date, end: Date, user?: any): Promise<Timesheet[]> {
    if (user) await this.validateBusinessAccess(businessId, user);
    return this.prisma.timesheet.findMany({
        where: {
            employee: {
  ...
  async handleAutoClockOut() {
    const logger = new Logger('AutoClockOut');
    const now = new Date();
    ...
    if (staleTimesheets.length === 0) return;
