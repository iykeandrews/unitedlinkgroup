...
async function runVerification() {
    ...
    const timeTrackingService = app.get(time_tracking_service_1.TimeTrackingService);
    // 1. Setup Data
    ...
    // 4. Verification
    const w2Stub = result.payStubs.find((s) => s.workerType === 'W2');
    const cStub = result.payStubs.find((s) => s.workerType === 'CONTRACTOR_1099');
    if (!w2Stub)
        throw new Error('Missing W2 PayStub');
    if (!cStub)
        throw new Error('Missing 1099 PayStub');
    ...
    if (Math.abs(w2Stub.grossPay - 1100) > 0.01)
        throw new Error(`W2 Gross Pay mismatch. Got ${w2Stub.grossPay}, expected 1100`);
    if (w2Stub.taxes <= 0)
        throw new Error('W2 Taxes should be calculated');
    ...
    if (Math.abs(cStub.grossPay - 300) > 0.01)
        throw new Error(`1099 Gross Pay mismatch. Got ${cStub.grossPay}, expected 300`);
    if (cStub.taxes !== 0)
        throw new Error('1099 Taxes should be 0');
    ...
    const w2Report = annualReport.w2Employees.find((e) => e.employee.id === employee.id);
    const cReport = annualReport.contractors.find((c) => c.employee.id === employee.id);
    if (!w2Report)
        throw new Error('Missing W2 Report entry');
    if (!cReport)
        throw new Error('Missing Contractor Report entry');
    ...
    if (Math.abs(w2Report.grossPay - 1100) > 0.01)
        throw new Error('W2 Report Gross mismatch');
    if (Math.abs(cReport.grossPay - 300) > 0.01)
        throw new Error('Contractor Report Gross mismatch');
    // 6. Validation Test
    console.log('Testing Validation Constraints...');
    try {
        // Correct signature: clockIn(employeeId, locationId, lat, lng, ip)
        await timeTrackingService.clockIn(employee.id, undefined, // locationId
        undefined, // lat
        undefined, // lng
        '127.0.0.1' // ip
        );
