...
import { 
  Wrench, MapPin, User, FileText, Calendar, DollarSign, Tag, Hash, 
  Shield, Shirt, Radio, Camera, Car, Box, AlertTriangle 
} from 'lucide-react';
...
const ASSET_CATEGORIES = [
  ...
  { id: 'PROTECTIVE_GEAR', label: 'Protective Equipment', icon: Shield },
  { id: 'COMM_DEVICES', label: 'Communication Devices', icon: Radio },
  { id: 'SURVEILLANCE', label: 'Surveillance & Gadgets', icon: Camera },
  { id: 'TRANSPORTATION', label: 'Transportation', icon: Car },
  { id: 'MISC', label: 'Misc. Security Equipment', icon: Box },
];
...
export default function AssetModal({ isOpen, onClose, onSuccess, initialData }: AssetModalProps) {
    ...
    <div className="grid grid-cols-2 gap-4">
      {ASSET_CATEGORIES.map((cat) => (
        <button
          ...
          className="flex flex-col items-center justify-center p-6 border-2 border-slate-200 dark:border-slate-700 rounded-xl hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-slate-700/50 transition-all group"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="col-span-2">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              required
              className="pl-9 block w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white uppercase"
        ...
        <div className="col-span-2">
          ...
          <input
            ...
            placeholder="Permit # or Reference ID"
          />
        </div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              type="number"
              step="0.01"
              className="pl-9 block w-full rounded-lg border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white"
  ...
  return (
    <Modal 
      isOpen={isOpen} 
      onClose={onClose} 
      title={step === 1 ? 'Select Asset Category' : `Add ${ASSET_CATEGORIES.find(c => c.id === category)?.label || 'Asset'}`}
      maxWidth={step === 1 ? 'max-w-3xl' : 'max-w-2xl'}
    >
      {step === 1 ? (
