...
import { AlertCircle, ChevronRight, Wallet, CheckSquare } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { useBusiness } from '../../context/business-context';
import { formatDate, getCountryConfig } from '../../lib/localization';
...
export function SidebarWidgets({ expiringQualifications, stats, hasModule }: SidebarWidgetsProps) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-gray-100 dark:divide-slate-700/50 max-h-80 overflow-y-auto custom-scrollbar">
            {expiringQualifications.map((qual: any) => {
              ...
              return (
                <div key={qual.id} className="p-4 hover:bg-gray-50 dark:hover:bg-slate-800/80 transition-colors group cursor-pointer" onClick={() => router.push(`/dashboard/people?employee=${qual.employee.id}&tab=qualifications`)}>
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <p className="text-xs text-gray-400 dark:text-slate-500">
                      Expires: {formatDate(qual.expiryDate, selectedBusiness?.country)}
                    </p>
      ...
      {/* Payroll Widget */}
      {hasModule('Pay') && (
        <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-2xl shadow-lg shadow-indigo-500/20 overflow-hidden text-white relative">
            ...
            {stats?.nextPayroll ? (
              <div>
                ...
                <p className="text-3xl font-bold mt-1 tracking-tight">
                  {formatDate(stats.nextPayroll.payDate, selectedBusiness?.country, { month: 'short', day: 'numeric' })}
                </p>
                
                <div className="mt-6 pt-6 border-t border-white/10">
                  <div className="flex justify-between text-sm mb-4">
                    <span className="text-indigo-200">Period Ends</span>
                    <span className="font-medium">{formatDate(stats.nextPayroll.periodEnd, selectedBusiness?.country)}</span>
                  </div>
                  <button 
                    onClick={() => router.push('/dashboard/payroll')}
                    className="w-full py-2.5 bg-white text-indigo-600 rounded-xl hover:bg-indigo-50 font-semibold text-sm transition-colors shadow-sm"
                  >
                    Run Payroll
                  </button>
                </div>
              ...
              <p className="text-indigo-200">No upcoming payroll</p>
            )}
          </div>
        </div>
      )}
