...
interface DashboardAlertsProps {
  expiringQualifications: any[];
  expiringSites: any[];
  showExpiringQualBanner: boolean;
  setShowExpiringQualBanner: (show: boolean) => void;
  setIsExpiringQualModalOpen: (open: boolean) => void;
  dismissedAlerts: string[];
  setDismissedAlerts: (fn: (prev: string[]) => string[]) => void;
  profile: any;
}
...
export function DashboardAlerts({
  ...
  setShowExpiringQualBanner,
  setIsExpiringQualModalOpen,
  dismissedAlerts,
  setDismissedAlerts,
  profile
  ...
  const hasExpiringSites = expiringSites.length > 0 && (profile?.role === 'BUSINESS_ADMIN' || profile?.role === 'SUPER_ADMIN');

  if (!hasExpiringQuals && !hasExpiringSites) return null;

  return (
    <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
      {/* Expiring Qualifications Banner */}
      {hasExpiringQuals && (
        <div className="relative overflow-hidden rounded-xl border border-red-200 dark:border-red-900/50 bg-white dark:bg-slate-800 shadow-sm">
          ...
          <div className="p-4 pl-5 flex items-start justify-between gap-4">
            <div className="flex gap-4">
              ...
              <div>
                ...
                <button
                  onClick={() => setIsExpiringQualModalOpen(true)}
                  className="mt-2 text-sm font-medium text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 flex items-center gap-1 group"
                >
                  View Details
            ...
            <button
              onClick={() => setShowExpiringQualBanner(false)}
              className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-700 hover:text-gray-500 dark:hover:text-gray-300 transition-colors"
            >
              <X size={16} />
            </button>
          </div>
            ...
            <div className="p-4 pl-5 flex items-start justify-between gap-4">
              <div className="flex gap-4">
                <div className={`p-2 rounded-lg shrink-0 ${isUrgent ? 'bg-red-100 dark:bg-red-900/20' : 'bg-amber-100 dark:bg-amber-900/20'}`}>
                  <AlertCircle className={`h-5 w-5 ${isUrgent ? 'text-red-600 dark:text-red-400' : 'text-amber-600 dark:text-amber-400'}`} />
                </div>
                <div>
                  ...
                  <button 
                    onClick={() => router.push(`/dashboard/security/sites/${site.id}`)}
                    className={`mt-2 px-3 py-1.5 text-xs font-medium rounded-lg inline-flex items-center gap-1 transition-colors ${
                      isUrgent 
              ...
              <button
                onClick={() => setDismissedAlerts(prev => [...prev, site.id])}
                className="p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-700 hover:text-gray-500 dark:hover:text-gray-300 transition-colors"
              >
                <X size={16} />
              </button>
