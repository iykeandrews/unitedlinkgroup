...
exports.default = DepartmentMembersModal;
const react_1 = require("react");
const react_2 = require("@headlessui/react");
const lucide_react_1 = require("lucide-react");
const api_1 = __importDefault(require("../../lib/api"));
function DepartmentMembersModal({ department, isOpen, onClose, onUpdate }) {
    const departmentId = department === null || department === void 0 ? void 0 : department.id;
    ...
    const [centerDialog, setCenterDialog] = (0, react_1.useState)(null);
    const [centerDialogBusy, setCenterDialogBusy] = (0, react_1.useState)(false);
    ...
    const openInfo = (title, message) => setCenterDialog({ kind: 'info', title, message });
    const openError = (title, message) => setCenterDialog({ kind: 'error', title, message });
    const openConfirm = (title, message, action, confirmLabel = 'Confirm', cancelLabel = 'Cancel') => setCenterDialog({ kind: 'confirm', title, message, action, confirmLabel, cancelLabel });
    const closeDialog = () => {
        if (centerDialogBusy)
            return;
    ...
    const handleAddMember = async (employeeId) => {
        try {
            ...
            await api_1.default.post(`/departments/${departmentId}/members`, { employeeId });
    ...
    const handleRemoveMember = async (employeeId) => {
        ...
        openConfirm('Remove member', `Remove ${label} from this department?`, async () => {
            try {
                if (!departmentId)
                    return;
                await api_1.default.delete(`/departments/${departmentId}/members/${employeeId}`);
                await fetchMembers();
                onUpdate();
    ...
    const handleAssignManager = async (employeeId) => {
        const member = members.find((m) => m.id === employeeId);
        const label = member ? `${member.firstName} ${member.lastName}`.trim() : 'this member';
        openConfirm('Assign manager', `Make ${label} the department manager?`, async () => {
            try {
                if (!departmentId)
                    return;
                await api_1.default.post(`/departments/${departmentId}/manager`, { employeeId });
                await fetchMembers();
                onUpdate();
    ...
    const handleRemoveManager = async () => {
        openConfirm('Remove manager', 'Remove the current manager from this department?', async () => {
            try {
                if (!departmentId)
                    return;
                await api_1.default.delete(`/departments/${departmentId}/manager`);
                await fetchMembers();
                onUpdate();
    ...
    const getWorkerTypeLabel = (emp) => {
        const isW2 = !!emp.w2Profile;
        const is1099 = !!emp.contractorProfile;
        if (isW2 && is1099)
            return <span className="px-2 py-0.5 rounded text-xs bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300">Dual</span>;
        if (isW2)
            return <span className="px-2 py-0.5 rounded text-xs bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">W-2</span>;
        if (is1099)
            return <span className="px-2 py-0.5 rounded text-xs bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300">1099</span>;
    ...
    return (<react_2.Transition show={isOpen} as={react_1.Fragment}>
      <react_2.Dialog as="div" className="relative z-50" onClose={onClose}>
        ...
        <div className="fixed inset-0 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4">
            <react_2.Transition.Child as={react_1.Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
              <react_2.Dialog.Panel className="w-full max-w-lg transform overflow-hidden rounded-2xl bg-white dark:bg-slate-800 p-6 shadow-xl transition-all border border-gray-200 dark:border-slate-700">
                <div className="flex justify-between items-center mb-6">
                  <react_2.Dialog.Title className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    <lucide_react_1.Building className="w-5 h-5 text-indigo-500"/>
                    Manage Members - {(department === null || department === void 0 ? void 0 : department.name) || ''}
                ...
                <div className="mb-6">
                  ...
                  {/* Search Results */}
                  {searchQuery && (<div className="mt-2 border border-gray-200 dark:border-slate-700 rounded-lg overflow-hidden bg-white dark:bg-slate-800 shadow-lg relative z-10">
            ...
            const isManager = (department === null || department === void 0 ? void 0 : department.managerId) === member.id;
            return (<div key={member.id} className={`flex items-center justify-between p-3 rounded-lg border ${isManager ? 'bg-indigo-50 border-indigo-200 dark:bg-indigo-900/20 dark:border-indigo-800' : 'bg-gray-50 border-gray-100 dark:bg-slate-700/50 dark:border-slate-700'}`}>
                            <div className="flex items-center gap-3">
                              <div className={`w-8 h-8 rounded-full flex items-center justify-center font-medium text-xs shadow-sm ${isManager ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-800 dark:text-indigo-200' : 'bg-white dark:bg-slate-600 text-gray-600 dark:text-gray-300'}`}>
                                {isManager ? <lucide_react_1.Crown className="w-4 h-4"/> : `${member.firstName[0]}${member.lastName[0]}`}
                              </div>
                              <div>
                                <div className="flex items-center gap-2">
                                  ...
                                  {isManager && (<span className="text-xs bg-indigo-100 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400 px-1.5 py-0.5 rounded font-medium">Manager</span>)}
                            ...
                            <div className="flex items-center gap-1">
                              {!isManager ? (<button onClick={() => handleAssignManager(member.id)} className="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-lg transition-colors" title="Assign as Manager">
                                  <lucide_react_1.BadgeCheck className="w-4 h-4"/>
                                </button>) : (<button onClick={handleRemoveManager} className="p-1.5 text-indigo-600 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors" title="Remove as Manager">
                                  <lucide_react_1.X className="w-4 h-4"/>
                                </button>)}
                              <button onClick={() => handleRemoveMember(member.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors" title="Remove from department">
      ...
      {centerDialog && (<div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onMouseDown={closeDialog}/>
          <div role="dialog" aria-modal="true" className="relative w-full max-w-md rounded-2xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-2xl">
            <div className="flex items-start justify-between gap-3 p-5 border-b border-gray-200 dark:border-slate-700">
              ...
              <button type="button" onClick={closeDialog} className="p-2 rounded-lg text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 hover:bg-gray-50 dark:hover:bg-slate-700/40 transition-colors" disabled={centerDialogBusy}>
                <lucide_react_1.X className="w-5 h-5"/>
              </button>
            </div>
            ...
            <div className="p-5 pt-0 flex items-center justify-end gap-3">
              {centerDialog.kind === 'confirm' ? (<>
                  <button type="button" onClick={closeDialog} disabled={centerDialogBusy} className="px-4 py-2 rounded-lg text-sm font-semibold border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-slate-600 disabled:opacity-50">
                    {centerDialog.cancelLabel || 'Cancel'}
                  </button>
                  <button type="button" disabled={centerDialogBusy} onClick={async () => {
                    if (!centerDialog.action)
                        return;
                    try {
                ...
                }} className={`px-4 py-2 rounded-lg text-sm font-semibold text-white disabled:opacity-50 ${centerDialog.title.toLowerCase().includes('remove') ? 'bg-red-600 hover:bg-red-700' : 'bg-indigo-600 hover:bg-indigo-700'}`}>
                    {centerDialog.confirmLabel || 'Confirm'}
                  </button>
                </>) : (<button type="button" onClick={closeDialog} className="px-4 py-2 rounded-lg text-sm font-semibold bg-indigo-600 hover:bg-indigo-700 text-white">
                  OK
                </button>)}
            </div>
          </div>
