...
exports.default = InventoryDetailPanel;
const react_1 = __importDefault(require("react"));
const lucide_react_1 = require("lucide-react");
const framer_motion_1 = require("framer-motion");
const inventory_context_1 = require("../../../context/inventory-context");
function InventoryDetailPanel({ isOpen, onClose, productId, variantId }) {
    const { products, stockLogs } = (0, inventory_context_1.useInventory)();
    const product = products.find(p => p.id === productId);
    const variant = product === null || product === void 0 ? void 0 : product.variants.find(v => v.id === variantId);
    if (!product || !variant)
        return null;
    ...
    return (<framer_motion_1.AnimatePresence>
      {isOpen && (<>
          <framer_motion_1.motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose} className="fixed inset-0 bg-black/20 backdrop-blur-sm z-40"/>
          <framer_motion_1.motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="fixed inset-y-0 right-0 z-50 w-full max-w-2xl bg-white dark:bg-slate-900 shadow-2xl border-l border-slate-200 dark:border-slate-700 overflow-y-auto">
            <div className="p-6">
              {/* Header */}
              <div className="flex justify-between items-start mb-8">
                ...
                <button onClick={onClose} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors">
                  <lucide_react_1.X size={24} className="text-slate-500"/>
                </button>
              </div>
              ...
              <div className="space-y-8">
                ...
                <section>
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                    <lucide_react_1.History size={20} className="text-indigo-500"/>
                  ...
                  <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
                    <table className="w-full text-sm text-left">
                      ...
                      <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                        {itemLogs.length > 0 ? itemLogs.map(log => (<tr key={log.id}>
                            <td className="p-3 text-slate-500">{new Date(log.timestamp).toLocaleDateString()}</td>
                            <td className="p-3 font-medium text-slate-700 dark:text-slate-300">
                              <span className={`px-2 py-0.5 rounded text-xs ${log.changeAmount > 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                {log.reason}
                              </span>
                            </td>
                ...
                <section>
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                    <lucide_react_1.Truck size={20} className="text-blue-500"/>
                    Recent Purchase Orders
                  </h3>
                  <div className="space-y-3">
                    {purchaseHistory.map(po => (<div key={po.id} className="flex justify-between items-center p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                        <div>
                ...
                {/* Locations (Mock) */}
                 <section>
                  <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                    <lucide_react_1.Package size={20} className="text-amber-500"/>
                    Stock Locations
                  </h3>
