...
export function ProductCard({ product, onQuickView, onEdit, isAdmin, viewMode = 'grid' }: ProductCardProps) {
  ...
  const secondaryImage = images.find(img => img.id !== displayImage?.id);
  ...
  if (viewMode === 'list') {
    return (
      <motion.div 
        className="group relative bg-white dark:bg-slate-800 rounded-lg overflow-hidden border border-slate-100 dark:border-slate-700 hover:shadow-xl transition-all duration-300 flex"
        ...
        <div className="p-6 flex flex-col justify-between flex-1">
                ...
                {salePrice ? (
                  <>
                    <span className="text-lg font-bold text-red-500">
                      {formatCurrency(salePrice, selectedBusiness?.currencyCode)}
                    </span>
              ...
              {uniqueColors.map((variant) => (
                <button
                  ...
                  className={`w-5 h-5 rounded-full border border-slate-200 dark:border-slate-600 transition-transform ${
                    (activeVariantId === variant.id || (!activeVariantId && uniqueColors[0].id === variant.id))
          ...
          <div className="flex items-center gap-3">
             ...
             <button 
               ...
               className={`p-2 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors ${inWishlist ? 'text-red-500 bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-900' : ''}`}
               title="Wishlist"
             >
             ...
             {isAdmin && (
               <button 
                 ...
                 className="ml-auto text-slate-500 hover:text-indigo-600 text-sm font-medium flex items-center gap-1"
               >
  ...
  return (
    <motion.div 
      className="group relative bg-white dark:bg-slate-800 rounded-lg overflow-hidden border border-slate-100 dark:border-slate-700 hover:shadow-xl transition-all duration-300"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      ...
      <div className="relative aspect-[3/4] overflow-hidden bg-slate-100 dark:bg-slate-700">
        {displayImage && (
          <Image
            ...
            alt={displayImage.alt}
            fill
            className={`object-cover transition-transform duration-700 ${isHovered ? 'scale-110' : 'scale-100'} ${isHovered && secondaryImage ? 'opacity-0' : 'opacity-100'}`}
          />
        )}
        ...
        {secondaryImage && (
          <Image
            ...
            fill
            className={`absolute inset-0 object-cover transition-opacity duration-700 ${isHovered ? 'opacity-100' : 'opacity-0'}`}
          />
        )}
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-2">
          {salePrice && (
            <span className="px-2 py-1 bg-red-500 text-white text-xs font-bold uppercase tracking-wider rounded-sm">
              Sale
            </span>
          )}
          {product.tags?.includes('New Arrival') && (
            <span className="px-2 py-1 bg-slate-900 text-white text-xs font-bold uppercase tracking-wider rounded-sm">
              New
            </span>
          )}
        </div>
        ...
        <div className={`absolute bottom-4 left-0 right-0 flex justify-center gap-3 transition-transform duration-300 ${isHovered ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'} lg:translate-y-0 lg:opacity-100`}>
          ...
          */}
          <button 
            onClick={() => onQuickView(product)}
            className="p-3 bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded-full shadow-lg hover:bg-slate-900 hover:text-white dark:hover:bg-slate-700 transition-colors"
            title="Quick View"
          >
            <Eye size={18} />
          </button>
          ...
          <button 
            onClick={handleToggleWishlist}
            className={`p-3 bg-white dark:bg-slate-800 rounded-full shadow-lg hover:bg-red-500 hover:text-white dark:hover:bg-red-500 transition-colors ${inWishlist ? 'text-red-500' : 'text-slate-900 dark:text-white'}`}
            title="Wishlist"
          >
            <Heart size={18} fill={inWishlist ? "currentColor" : "none"} />
          </button>
        ...
        {isAdmin && (
          <button 
            ...
            className="absolute top-3 right-3 p-2 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-full text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400"
          >
        ...
        <div className="flex items-center gap-1.5">
          {uniqueColors.map((variant) => (
            <button
              key={variant.id}
              onClick={() => setActiveVariantId(variant.id)}
              className={`w-4 h-4 rounded-full border border-slate-200 dark:border-slate-600 transition-transform ${
                (activeVariantId === variant.id || (!activeVariantId && uniqueColors[0].id === variant.id))
                  ? 'scale-125 ring-1 ring-slate-400 ring-offset-1' 
                  : 'hover:scale-110'
              }`}
          ...
          {variants.length > 5 && (
            <span className="text-xs text-slate-400 ml-1">+{variants.length - 5}</span>
          )}
        </div>
      </div>
    </motion.div>
