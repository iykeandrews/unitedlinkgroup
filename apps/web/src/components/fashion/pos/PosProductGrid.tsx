...
import { Search, Filter, History, ScanBarcode } from 'lucide-react';
...
export function PosProductGrid({ products, onProductSelect, onHistoryClick }: PosProductGridProps) {
  const [isScanning, setIsScanning] = useState(false);
  const [scanInput, setScanInput] = useState('');
  const { searchQuery, setSearchQuery, selectedCategory, setSelectedCategory, addToCart } = usePos();
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900/50 relative">
      {/* Scanner Overlay */}
      {isScanning && (
        <div className="absolute inset-0 z-20 bg-black/50 backdrop-blur-sm flex items-start justify-center pt-20">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-2xl w-full max-w-md mx-4">
                <h3 className="text-lg font-bold mb-4 flex items-center gap-2 dark:text-white">
                    <ScanBarcode /> Scan Item
                </h3>
                <form onSubmit={handleScan}>
                    <input
                        ...
                        className="w-full px-4 py-3 bg-slate-100 dark:bg-slate-700 border-2 border-indigo-500 rounded-xl focus:outline-none text-lg mb-4"
        ...
        <div className="flex gap-4 mb-4">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-3 bg-slate-100 dark:bg-slate-700 border-none rounded-xl focus:ring-2 focus:ring-indigo-500 dark:text-white"
              value={searchQuery}
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-medium hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
          >
            <ScanBarcode size={20} />
            Scan
          </button>
