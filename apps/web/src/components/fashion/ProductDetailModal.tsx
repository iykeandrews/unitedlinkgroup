import { X, Heart, ShoppingBag, Share2, Ruler } from 'lucide-react';
...
export function ProductDetailModal({ product, onClose }: ProductDetailModalProps) {
  ...
  const [selectedImageId, setSelectedImageId] = useState(product.images.find(img => img.isPrimary)?.id || product.images[0].id);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <motion.div 
        ...
        exit={{ opacity: 0, y: 50 }}
        className="relative w-full max-w-5xl bg-white dark:bg-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row max-h-[90vh]"
      >
        <button 
          onClick={onClose}
          className="absolute top-4 right-4 z-10 p-2 bg-white/80 dark:bg-black/20 backdrop-blur-sm rounded-full hover:bg-white dark:hover:bg-black/40 transition-colors"
        >
          <X size={20} />
        </button>
            ...
            {product.images.map((img) => (
              <button
                ...
                className={`relative w-20 h-20 flex-shrink-0 rounded-lg overflow-hidden border-2 transition-colors ${
                  selectedImageId === img.id ? 'border-indigo-600' : 'border-transparent'
                }`}
              >
        ...
        <div className="w-full md:w-1/2 p-6 md:p-8 lg:p-10 overflow-y-auto">
              ...
              <div className="flex flex-wrap gap-3">
                {uniqueColors.map((variant) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full border border-slate-200 dark:border-slate-600 focus:outline-none ring-2 ring-offset-2 ${
                      selectedColor === variant.color ? 'ring-indigo-600' : 'ring-transparent'
                    }`}
          ...
          <div className="flex gap-4 mb-8">
            <button 
              ...
              className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-6 rounded-xl transition-colors flex items-center justify-center gap-2 shadow-lg shadow-indigo-500/20"
            >
              <ShoppingBag size={20} />
              Add to Cart
            </button>
