...
import { DesignProject, DesignStatus } from '../../../types/fashion';
...
const COLUMNS: { id: DesignStatus; title: string; color: string }[] = [
...
export default function DesignProjectBoard({ projects, onSelectProject }: DesignProjectBoardProps) {
  return (
    <div className="flex gap-6 overflow-x-auto pb-6 h-[calc(100vh-250px)]">
      {COLUMNS.map((column) => {
        ...
        return (
          <div key={column.id} className="flex-none w-80 bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar">
              {columnProjects.map((project) => (
                <div 
                  ...
                  className="bg-white dark:bg-slate-800 p-3 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md cursor-pointer transition-all"
                >
                  {project.thumbnailUrl && (
                    <div className="h-32 mb-3 rounded bg-slate-100 overflow-hidden">
