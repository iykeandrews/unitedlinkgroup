...
exports.default = TechPackView;
const react_1 = __importDefault(require("react"));
const lucide_react_1 = require("lucide-react");
function TechPackView({ techPack }) {
    if (!techPack) {
        return (<div className="flex flex-col items-center justify-center py-20 text-slate-400">
        <lucide_react_1.Layers className="w-12 h-12 mb-4 opacity-50"/>
    ...
    return (<div className="space-y-8">
      ...
      <div className="flex justify-end">
        <button onClick={() => window.print()} className="flex items-center gap-2 px-4 py-2 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
          ...
          <div className="aspect-[3/4] bg-white dark:bg-slate-800 rounded-lg flex items-center justify-center border border-dashed border-slate-300 dark:border-slate-600">
            {techPack.sketches.front ? (<img src={techPack.sketches.front} alt="Front Sketch" className="w-full h-full object-contain"/>) : <span className="text-slate-400">Upload Sketch</span>}
          </div>
        </div>
        <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
          <h3 className="font-semibold text-slate-900 dark:text-white mb-4">Back Sketch</h3>
          <div className="aspect-[3/4] bg-white dark:bg-slate-800 rounded-lg flex items-center justify-center border border-dashed border-slate-300 dark:border-slate-600">
            {techPack.sketches.back ? (<img src={techPack.sketches.back} alt="Back Sketch" className="w-full h-full object-contain"/>) : <span className="text-slate-400">Upload Sketch</span>}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
        <h3 className="font-semibold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
          <lucide_react_1.Scissors className="w-5 h-5 text-indigo-500"/>
          Construction & Specs
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
        <h3 className="font-semibold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
          <lucide_react_1.Ruler className="w-5 h-5 text-indigo-500"/>
       ...
       <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700">
        <h3 className="font-semibold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
          <lucide_react_1.Layers className="w-5 h-5 text-indigo-500"/>
