...
exports.ShiftCard = ShiftCard;
const react_1 = __importDefault(require("react"));
const core_1 = require("@dnd-kit/core");
const lucide_react_1 = require("lucide-react");
const date_fns_1 = require("date-fns");
function ShiftCard({ shift, onEdit, draggable = true, isMine = false }) {
    var _a, _b, _c, _d;
    ...
    const hasPendingApps = shift.status === 'OPEN' && ((_a = shift.applications) === null || _a === void 0 ? void 0 : _a.some(a => a.status === 'PENDING'));
    const hasCallout = !!shift.callout;
    const calloutNoticeAt = ((_b = shift.callout) === null || _b === void 0 ? void 0 : _b.noticeAt) ? new Date(shift.callout.noticeAt) : null;
    const minutesNoticeBeforeStart = calloutNoticeAt ? (0, date_fns_1.differenceInMinutes)(start, calloutNoticeAt) : null;
    ...
    return (<div ref={setNodeRef} style={style} {...listeners} {...attributes} onClick={(e) => {
            e.stopPropagation();
            onEdit(shift);
      ...
      {isDraft && (<div className="absolute top-1 left-1 text-[9px] px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-100 font-semibold tracking-wide">
          DRAFT
        </div>)}
      {(hasCallout || isLateNotice) && (<div className="absolute top-1 right-1 flex items-center gap-1">
          {hasCallout && (isCalloutOpen ? <lucide_react_1.UserRoundX className="w-3 h-3 text-red-700 dark:text-red-300"/> : <lucide_react_1.UserRoundCheck className="w-3 h-3 text-slate-700 dark:text-slate-200"/>)}
          {isLateNotice && <lucide_react_1.AlertCircle className="w-3 h-3 text-red-700 dark:text-red-300"/>}
        </div>)}
      <div className={`font-semibold ${nameColors} break-words whitespace-normal leading-snug`}>
        {isMine ? (shift.employee ? `${shift.employee.firstName} ${shift.employee.lastName}`.trim() : 'Me') : (isEmpty ? 'Open shift' : `${((_c = shift.employee) === null || _c === void 0 ? void 0 : _c.firstName) || ''} ${((_d = shift.employee) === null || _d === void 0 ? void 0 : _d.lastName) || ''}`.trim())}
      </div>
      <div className={`${timeColors} flex items-center gap-1 text-[10px]`}>
        {(0, date_fns_1.format)(start, 'h:mm a')} - {end ? (0, date_fns_1.format)(end, 'h:mm a') : 'In Progress'}
      </div>
