...
exports.default = ComplianceDocumentsPage;
...
function ComplianceDocumentsPage() {
    ...
    const [ownerFilter, setOwnerFilter] = (0, react_1.useState)('ALL');
    ...
    const canEdit = (profile === null || profile === void 0 ? void 0 : profile.role) === 'SUPER_ADMIN' || (profile === null || profile === void 0 ? void 0 : profile.role) === 'BUSINESS_ADMIN';
    const load = async () => {
        var _a, _b;
        setLoading(true);
        try {
            const [profileRes, docsRes, employeesRes] = await Promise.all([
                ...
                api_1.default.get('/compliance-documents'),
                api_1.default.get('/employees', { params: { status: 'ACTIVE' } }).catch(() => ({ data: [] })),
            ]);
            ...
            sonner_1.toast.error(((_b = (_a = e === null || e === void 0 ? void 0 : e.response) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.message) || 'Failed to load compliance documents');
        }
        finally {
            setLoading(false);
        }
    ...
    const openEdit = (d) => {
        ...
        setFormData({
            ...
            version: d.version || '',
            effectiveDate: d.effectiveDate ? d.effectiveDate.slice(0, 10) : (0, date_fns_1.format)(new Date(), 'yyyy-MM-dd'),
    ...
    const saveDoc = async () => {
        ...
        try {
            ...
            if (editingId) {
                await api_1.default.patch(`/compliance-documents/${editingId}`, payload);
                sonner_1.toast.success('Document updated');
            }
            else {
                await api_1.default.post('/compliance-documents', payload);
                sonner_1.toast.success('Document created');
            }
            ...
            sonner_1.toast.error(((_b = (_a = e === null || e === void 0 ? void 0 : e.response) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.message) || 'Failed to save document');
        }
        finally {
            setSaving(false);
        }
    ...
    const deleteDoc = async (id) => {
        var _a, _b;
        try {
            await api_1.default.delete(`/compliance-documents/${id}`);
            sonner_1.toast.success('Document removed');
            await load();
        }
        catch (e) {
            sonner_1.toast.error(((_b = (_a = e === null || e === void 0 ? void 0 : e.response) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.message) || 'Failed to remove document');
        }
    };
    ...
    return (<div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          {canEdit && (<button onClick={openAdd} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors shadow-sm">
              <lucide_react_1.Plus className="w-4 h-4"/>
              New Document
            </button>)}
          <button onClick={exportCSV} className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        <StatCard title="Total" value={`${stats.total}`} icon={lucide_react_1.ShieldCheck}/>
        <StatCard title="Active" value={`${stats.active}`} icon={lucide_react_1.ShieldCheck}/>
        <StatCard title="Draft" value={`${stats.draft}`} icon={lucide_react_1.FileText}/>
        <StatCard title="Archived" value={`${stats.archived}`} icon={lucide_react_1.FileText}/>
        <StatCard title="Review Due (30d)" value={`${stats.dueSoon}`} icon={lucide_react_1.AlertCircle}/>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 space-y-4 md:space-y-0 md:flex md:items-center md:justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" placeholder="Search title, tags, owner..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" value={query} onChange={(e) => setQuery(e.target.value)}/>
        </div>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-200">
              {filtered.length === 0 ? (<tr>
                  <td colSpan={7} className="px-6 py-8 text-center text-gray-500">
                ...
                </tr>) : (filtered.map((d) => (<tr key={d.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${d.status === 'ACTIVE' ? 'bg-green-100 text-green-800 border-green-200' : d.status === 'DRAFT' ? 'bg-amber-100 text-amber-800 border-amber-200' : 'bg-gray-100 text-gray-800 border-gray-200'}`}>{d.status}</span>
                    </td>
                    <td className="px-6 py-4">
                      {d.ownerEmployee ? (<div className="flex items-center gap-1 text-xs text-gray-700">
                    ...
                    <td className="px-6 py-4">
                      {d.reviewDate ? (<div className="flex items-center gap-1 text-xs text-gray-700">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-3">
                        {d.fileUrl && (<a href={(0, file_url_1.resolveFileUrl)(d.fileUrl)} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-gray-700 transition-colors" title="View Document">
      ...
      <Modal_1.Modal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); resetForm(); }} title={editingId ? 'Update compliance document' : 'Add compliance document'} maxWidth="max-w-2xl">
...
const StatCard = ({ title, value, icon: Icon }) => (<div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
