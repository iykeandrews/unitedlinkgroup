  ...
  useEffect(() => {
    if (selectedBusiness) {
        if (selectedBusiness.modules) {
            ...
            const defaultModules = MODULES.filter(m => m.category === 'Core').map(m => m.id);
            
            if (selectedBusiness.industry) {
                const industry = selectedBusiness.industry.toLowerCase();
                const businessType = selectedBusiness.businessType?.toLowerCase() || '';

                if (industry.includes('security')) {
                    defaultModules.push(...MODULES.filter(m => m.category === 'Security').map(m => m.id));
                }
                if (industry.includes('health') || industry.includes('medical') || industry.includes('care')) {
                    defaultModules.push(...MODULES.filter(m => m.category === 'Healthcare').map(m => m.id));
                }
                if (industry.includes('hotel') || industry.includes('motel') || businessType.includes('hospitality')) {
                    defaultModules.push(...MODULES.filter(m => m.category === 'Hotel').map(m => m.id));
                }
                if (industry.includes('agri') || industry.includes('farm') || industry.includes('harvest')) {
                    defaultModules.push(...MODULES.filter(m => m.category === 'Agriculture').map(m => m.id));
                }
                if (industry.includes('property') || industry.includes('real estate') || industry.includes('housing')) {
                    defaultModules.push(...MODULES.filter(m => m.category === 'Rental').map(m => m.id));
                }
                if (industry.includes('fashion') || businessType.includes('fashion') || industry.includes('clothing') || industry.includes('textile') || industry.includes('apparel') || industry.includes('style') || industry.includes('modeling')) {
                    defaultModules.push(...MODULES.filter(m => m.category === 'Fashion').map(m => m.id));
                }
            }
    ...
    <div className="max-w-4xl mx-auto py-8 px-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        ...
        <button
          onClick={handleSave}
          disabled={isLoading}
          className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 font-medium shadow-sm"
        >
          {isLoading ? (
            'Saving...'
          ) : isSaved ? (
