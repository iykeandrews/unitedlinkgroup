...
import { CheckCircle2, Download, ExternalLink, FileText, Plus, Search, Trash2 } from 'lucide-react';
...
type DocCategory = 'POLICY' | 'SOP' | 'TRAINING' | 'OSHA' | 'HR' | 'SECURITY' | 'OTHER';
...
export default function CompliancePage() {
  ...
  const [ownerEmployeeId, setOwnerEmployeeId] = useState<string>('all');
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (ownerEmployeeId !== 'all') params.ownerEmployeeId = ownerEmployeeId === 'unassigned' ? '' : ownerEmployeeId;

      const [docsRes, empRes] = await Promise.allSettled([
        api.get('/compliance-documents', { params: { ...params, businessId: selectedBusiness?.id } }),
      ...
      const msg = error?.response?.data?.message || 'Failed to load compliance documents';
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  }, [category, ownerEmployeeId, search, selectedBusiness?.id, status]);
  ...
  const stats = useMemo(() => {
    ...
    const needsReview = documents.filter(d => d.status !== 'ARCHIVED' && d.reviewDate && isBefore(new Date(d.reviewDate), soon)).length;
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editor.id) {
        await api.patch(`/compliance-documents/${editor.id}`, payload, { params: { businessId: selectedBusiness?.id } });
        toast.success('Compliance document updated');
      } else {
        await api.post('/compliance-documents', payload, { params: { businessId: selectedBusiness?.id } });
        toast.success('Compliance document created');
      }
      setEditor(prev => ({ ...prev, open: false }));
      fetchData();
    } catch (error: any) {
      const msg = error?.response?.data?.message || 'Failed to save compliance document';
      toast.error(msg);
  ...
  const performDelete = async () => {
    ...
    try {
      await api.delete(`/compliance-documents/${deleteConfirm.id}`, { params: { businessId: selectedBusiness?.id } });
      toast.success('Compliance document deleted');
      setDeleteConfirm({ isOpen: false, id: undefined, isLoading: false });
      fetchData();
    } catch (error: any) {
      const msg = error?.response?.data?.message || 'Failed to delete compliance document';
      toast.error(msg);
      setDeleteConfirm(prev => ({ ...prev, isLoading: false }));
    }
  };

  const statusPill = (s: DocStatus) => {
  ...
  const categoryPill = (c: DocCategory) => {
    if (c === 'SECURITY') return 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300';
    if (c === 'OSHA') return 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300';
    if (c === 'TRAINING') return 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300';
    if (c === 'HR') return 'bg-sky-100 text-sky-800 dark:bg-sky-900/30 dark:text-sky-300';
    return 'bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-200';
  };
    ...
    <div className="py-8">
      <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          <StatCard label="Total" value={stats.total} icon={<FileText className="w-4 h-4" />} />
          <StatCard label="Active" value={stats.active} icon={<CheckCircle2 className="w-4 h-4" />} />
          <StatCard label="Draft" value={stats.draft} icon={<FileText className="w-4 h-4" />} />
          <StatCard label="Needs review (30d)" value={stats.needsReview} icon={<FileText className="w-4 h-4" />} danger />
          <StatCard label="Archived" value={stats.archived} icon={<FileText className="w-4 h-4" />} />
        </div>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                {loading ? (
                  ...
                  documents.map((d) => {
                    ...
                    const reviewSoon = d.status !== 'ARCHIVED' && reviewDate && isBefore(reviewDate, addDays(new Date(), 30));
                    return (
                      <tr
                        key={d.id}
                        className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors cursor-pointer"
                        onClick={() => (canManage ? openEdit(d) : null)}
                      >
                        <td className="px-6 py-4">
                          ...
                          <div className="text-xs text-slate-500 flex flex-wrap gap-x-3 gap-y-1 mt-1">
                            ...
                            {d.effectiveDate ? <span>Effective {format(new Date(d.effectiveDate), 'd MMM yyyy')}</span> : null}
                        ...
                        <td className="px-6 py-4 text-sm">
                          {reviewDate ? (
                            <span className={`${reviewSoon ? 'text-red-600 dark:text-red-400 font-semibold' : 'text-slate-700 dark:text-slate-200'}`}>
                        ...
                        <td className="px-6 py-4">
                          {d.fileUrl ? (
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                window.open(d.fileUrl as string, '_blank', 'noopener,noreferrer');
                              }}
      ...
      <Modal isOpen={editor.open} onClose={() => setEditor(prev => ({ ...prev, open: false }))} title={editor.id ? 'Edit document' : 'New document'} maxWidth="max-w-3xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder="e.g. Site access policy, Patrol SOP, Incident reporting training"
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setEditor(prev => ({ ...prev, category: e.target.value as any }))}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setEditor(prev => ({ ...prev, effectiveDate: e.target.value }))}
              className="mt-1 w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setEditor(prev => ({ ...prev, reviewDate: e.target.value }))}
              className="mt-1 w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg"
            />
          </div>
          ...
          <div>
            ...
            <select
              value={editor.ownerEmployeeId}
              onChange={(e) => setEditor(prev => ({ ...prev, ownerEmployeeId: e.target.value }))}
              className="mt-1 w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg"
            >
              <option value="unassigned">No owner</option>
              {employees.map(e => (
          ...
          <div className="flex items-center gap-2 mt-6">
            <input
              id="ack-required"
              type="checkbox"
              checked={editor.acknowledgementRequired}
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              className="mt-1 w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg"
              placeholder="e.g. patrol, incident, client-site"
            />
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="mt-1 flex items-center gap-2">
              ...
              {editor.fileUrl.trim() ? (
                <button
                  type="button"
                  onClick={() => window.open(editor.fileUrl.trim(), '_blank', 'noopener,noreferrer')}
                  className="px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200"
                  title="Open file"
                >
                  <ExternalLink className="w-4 h-4" />
                </button>
      ...
      <ConfirmModal
        ...
        message="Delete this compliance document? This action cannot be undone."
        confirmText="Delete"
        cancelText="Cancel"
        variant="danger"
