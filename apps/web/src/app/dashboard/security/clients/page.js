...
function ClientsPage() {
    ...
    const filteredClients = clients.filter(client => {
        ...
        return client.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
            ((_a = client.contactPerson) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(searchQuery.toLowerCase())) ||
            ((_b = client.email) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes(searchQuery.toLowerCase()));
    });
    const handleExport = () => {
    ...
    return (<div className="py-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-2">
            ...
            <button onClick={() => router.push('/dashboard/security/clients/new')} className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        ...
        <div className="p-4 border-b border-slate-200 dark:border-slate-700">
          <div className="relative max-w-md">
            ...
            <input type="text" placeholder="Search clients..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"/>
          </div>
        </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {loading ? (<tr>
                ...
                </tr>) : filteredClients.length === 0 ? (<tr>
                  <td colSpan={7} className="px-6 py-8 text-center text-slate-500">No clients found.</td>
                </tr>) : (filteredClients.map((client) => {
            var _a;
            return (<tr key={client.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 cursor-pointer transition-colors" onClick={() => router.push(`/dashboard/security/clients/${client.id}`)}>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="flex items-center gap-2 text-xs text-slate-500">
                          <lucide_react_1.Mail size={12}/> {client.email}
                        </div>
                        <div className="flex items-center gap-2 text-xs text-slate-500">
                          <lucide_react_1.Phone size={12}/> {client.phone}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <span className="text-sm font-medium text-slate-900 dark:text-white">
                        {((_a = client._count) === null || _a === void 0 ? void 0 : _a.locations) || 0}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <button className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                        <lucide_react_1.MoreHorizontal size={20}/>
                      </button>
                    </td>
                  </tr>);
        }))}
