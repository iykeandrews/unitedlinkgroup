...
const AddTeamMemberModal_1 = require("../../../components/AddTeamMemberModal");
const EmployeeProfileModal_1 = require("../../../components/EmployeeProfileModal");
const EmployeeSelfProfilePage_1 = __importDefault(require("../../../components/people/EmployeeSelfProfilePage"));
const generateEmployeePdf_1 = require("../../../utils/generateEmployeePdf");
...
function AdminPeoplePageContent() {
    ...
    const fetchEmployees = async () => {
        try {
            ...
            if (selectedEmployee) {
                const updated = res.data.find((e) => e.id === selectedEmployee.id);
                if (updated)
    ...
    return (<div className="flex flex-col h-full w-full bg-slate-50/50 dark:bg-slate-950/50">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-6 rounded-2xl border border-gray-200/50 dark:border-slate-700/50 shadow-sm">
        ...
        <button onClick={() => setIsAddModalOpen(true)} className="group relative px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-medium transition-all shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/40 flex items-center gap-2 overflow-hidden">
      ...
      <AddTeamMemberModal_1.AddTeamMemberModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} onSuccess={fetchEmployees}/>

      <EmployeeProfileModal_1.EmployeeProfileModal isOpen={!!selectedEmployee} onClose={() => setSelectedEmployee(null)} employee={selectedEmployee} onUpdate={fetchEmployees} initialTab={searchParams.get('tab') || undefined} highlightQualificationId={searchParams.get('qualificationId') || undefined}/>
      ...
      <div className="bg-white dark:bg-slate-800/50 backdrop-blur-xl border border-gray-200 dark:border-slate-700/50 p-4 rounded-2xl flex flex-col lg:flex-row justify-between items-center gap-4 shadow-xl">
        <div className="flex items-center gap-3 w-full lg:w-auto overflow-x-auto pb-2 lg:pb-0">
          ...
          <div className="relative group min-w-[240px]">
            <lucide_react_1.Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 dark:text-slate-400 group-focus-within:text-indigo-500 dark:group-focus-within:text-indigo-400 transition-colors"/>
            <input type="text" placeholder="Search by name, email, or role..." value={searchQuery} onChange={(e) => {
          ...
          <div className="flex bg-gray-50 dark:bg-slate-900/50 rounded-lg p-1 border border-gray-200 dark:border-slate-700">
            <button onClick={() => setViewMode('grid')} className={`p-2 rounded-md transition-all ${viewMode === 'grid' ? 'bg-white dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-gray-400 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white'}`}>
              <lucide_react_1.LayoutGrid className="w-4 h-4"/>
            </button>
            <button onClick={() => setViewMode('list')} className={`p-2 rounded-md transition-all ${viewMode === 'list' ? 'bg-white dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-gray-400 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white'}`}>
              <lucide_react_1.List className="w-4 h-4"/>
            </button>
          </div>

          {/* Filters */}
          <react_2.Popover className="relative">
            <react_2.Popover.Button className="flex items-center gap-2 px-4 py-2.5 bg-gray-50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-700 text-gray-700 dark:text-slate-300 rounded-xl text-sm font-medium hover:bg-gray-100 dark:hover:bg-slate-800 hover:border-gray-300 dark:hover:border-slate-600 transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500/20">
              <lucide_react_1.Filter className="w-4 h-4"/>
              <span>Filters</span>
              {(roleFilter !== 'ALL' || statusFilter !== 'ALL') && (<span className="bg-indigo-500 text-white text-[10px] px-1.5 py-0.5 rounded-full">
            ...
            <react_2.Transition as={react_1.Fragment} enter="transition ease-out duration-200" enterFrom="opacity-0 translate-y-1" enterTo="opacity-100 translate-y-0" leave="transition ease-in duration-150" leaveFrom="opacity-100 translate-y-0" leaveTo="opacity-0 translate-y-1">
              <react_2.Popover.Panel className="absolute right-0 z-30 mt-2 w-72 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-gray-200 dark:border-slate-700 p-4">
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="space-y-1">
                      {['ALL', 'ACTIVE', 'INACTIVE', 'TERMINATED'].map((status) => (<button key={status} onClick={() => setStatusFilter(status)} className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg transition-colors ${statusFilter === status
                ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400'
                : 'text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700/50'}`}>
                          <span className="capitalize">{status.toLowerCase()}</span>
                          {statusFilter === status && <lucide_react_1.Check className="w-4 h-4"/>}
                        </button>))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {['ALL', 'BUSINESS_ADMIN', 'MANAGER', 'SECURITY_OFFICER'].map((role) => (<button key={role} onClick={() => setRoleFilter(role)} className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded-lg transition-colors ${roleFilter === role
                ? 'bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400'
                : 'text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700/50'}`}>
                          <span className="capitalize">{role === 'BUSINESS_ADMIN' ? 'Admin' : role.toLowerCase().replace('_', ' ')}</span>
                          {roleFilter === role && <lucide_react_1.Check className="w-4 h-4"/>}
                        </button>))}
                    </div>
                  </div>

                  {(roleFilter !== 'ALL' || statusFilter !== 'ALL') && (<div className="pt-2 border-t border-gray-100 dark:border-slate-700/50">
                      <button onClick={() => {
                setRoleFilter('ALL');
                setStatusFilter('ALL');
        ...
        <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-slate-400">
          ...
          <div className="flex gap-1">
            <button onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} disabled={currentPage === 1} className="p-2 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-lg disabled:opacity-30 disabled:hover:bg-transparent transition-colors">
              <lucide_react_1.ChevronLeft className="w-4 h-4"/>
            </button>
            <button onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} disabled={currentPage === totalPages || totalPages === 0} className="p-2 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-lg disabled:opacity-30 disabled:hover:bg-transparent transition-colors">
      ...
      <div className="min-h-[500px]">
        {paginatedEmployees.length === 0 ? (<div className="flex flex-col items-center justify-center h-64 text-slate-500 bg-slate-800/30 rounded-2xl border border-slate-700/50 border-dashed">
            <lucide_react_1.User className="w-12 h-12 mb-4 opacity-20"/>
            <p className="text-lg font-medium">No team members found</p>
            <p className="text-sm">Try adjusting your search or filters</p>
          </div>) : (<framer_motion_1.AnimatePresence mode="wait">
            {viewMode === 'grid' ? (<framer_motion_1.motion.div key="grid" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {paginatedEmployees.map((employee, index) => {
                    const rates = getPayRates(employee);
                    return (<framer_motion_1.motion.div key={employee.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className={`
                      ...
                      <div className="absolute top-4 left-4 z-10">
                        <input type="checkbox" className="w-5 h-5 rounded border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900/50 text-indigo-600 dark:text-indigo-500 focus:ring-offset-0 focus:ring-indigo-500 transition-colors cursor-pointer" checked={selectedIds.has(employee.id)} onChange={() => toggleSelection(employee.id)}/>
                      </div>

                      {/* Download Button */}
                      <button onClick={(e) => {
                            e.stopPropagation();
                            (0, generateEmployeePdf_1.generateEmployeePdf)(employee, selectedBusiness);
                        }} className="absolute top-4 right-4 p-2 rounded-lg bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm border border-transparent hover:bg-white dark:hover:bg-slate-800 hover:border-gray-200 dark:hover:border-slate-700 text-gray-400 hover:text-indigo-600 dark:text-slate-500 dark:hover:text-indigo-400 transition-all z-20" title="Download Profile PDF">
                      ...
                      <div className="p-6 flex flex-col items-center text-center cursor-pointer" onClick={() => setSelectedEmployee(employee)}>
                        <div className="relative mb-4 w-20 h-20">
                          {employee.profileImageUrl ? (<img src={(0, file_url_1.resolveFileUrl)(employee.profileImageUrl)} alt={`${employee.firstName} ${employee.lastName}`} className="w-20 h-20 rounded-2xl shadow-lg group-hover:scale-105 transition-transform duration-300 object-cover"/>) : (<image_1.default src={`https://api.dicebear.com/7.x/initials/svg?seed=${employee.firstName} ${employee.lastName}`} alt={`${employee.firstName} ${employee.lastName}`} fill className="rounded-2xl shadow-lg group-hover:scale-105 transition-transform duration-300 object-cover"/>)}
                        ...
                        <div className="w-full grid grid-cols-2 gap-2 mb-6">
                          ...
                          <div className="bg-gray-50 dark:bg-slate-900/50 rounded-lg p-2 border border-gray-100 dark:border-slate-700/50">
                            ...
                            <div className="text-sm font-semibold text-gray-900 dark:text-white">{(0, localization_1.formatCurrency)(rates.weekday, selectedBusiness === null || selectedBusiness === void 0 ? void 0 : selectedBusiness.currencyCode)}</div>
                          </div>
                        </div>
                        ...
                        <div className="w-full pt-4 border-t border-gray-100 dark:border-slate-700/50 flex items-center justify-between text-sm">
                          <span className="text-gray-500 flex items-center gap-1.5">
                            <lucide_react_1.Briefcase className="w-3 h-3"/>
              ...
              </framer_motion_1.motion.div>) : (<framer_motion_1.motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-2">
                ...
                <div className="grid grid-cols-12 gap-4 px-6 py-4 bg-gray-50/80 dark:bg-slate-800/80 backdrop-blur-md border border-gray-200 dark:border-slate-700/50 rounded-xl mb-4 text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider shadow-sm">
                  <div className="col-span-1 flex items-center">
                    <input type="checkbox" className="w-4 h-4 rounded border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900/50 text-indigo-600 dark:text-indigo-500 focus:ring-indigo-500 cursor-pointer" checked={selectedIds.size === paginatedEmployees.length && paginatedEmployees.length > 0} onChange={toggleAll}/>
                  </div>
                  <div className="col-span-3 flex items-center gap-2 group cursor-pointer hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                    Employee
                    <lucide_react_1.ChevronRight className="w-3 h-3 rotate-90 opacity-0 group-hover:opacity-100 transition-opacity"/>
                  </div>
                ...
                {paginatedEmployees.map((employee, index) => {
                    var _a;
                    const rates = getPayRates(employee);
                    return (<framer_motion_1.motion.div key={employee.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.03 }} className={`
                      ...
                      <div className="col-span-1">
                        <input type="checkbox" className="rounded border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900/50 text-indigo-600 dark:text-indigo-500 focus:ring-indigo-500" checked={selectedIds.has(employee.id)} onChange={() => toggleSelection(employee.id)}/>
                      </div>
                      <div className="col-span-3 flex items-center gap-3 cursor-pointer" onClick={() => setSelectedEmployee(employee)}>
                        <div className="relative w-10 h-10 rounded-full overflow-hidden">
                          {employee.profileImageUrl ? (<img src={(0, file_url_1.resolveFileUrl)(employee.profileImageUrl)} alt={`${employee.firstName} ${employee.lastName}`} className="w-10 h-10 object-cover"/>) : (<image_1.default src={`https://api.dicebear.com/7.x/initials/svg?seed=${employee.firstName} ${employee.lastName}`} alt={`${employee.firstName} ${employee.lastName}`} fill className="object-cover"/>)}
                        </div>
                        <div>
                      ...
                      <div className="col-span-2">
                         <div className="flex items-center gap-2">
                           ...
                           <span className="text-sm text-gray-700 dark:text-slate-300 capitalize">{((_a = employee.status) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || 'Active'}</span>
                         </div>
                      </div>
                      <div className="col-span-2 text-sm text-gray-900 dark:text-slate-300">
                        {(0, localization_1.formatCurrency)(rates.weekday, selectedBusiness === null || selectedBusiness === void 0 ? void 0 : selectedBusiness.currencyCode)} <span className="text-gray-500 text-xs">/hr</span>
                      </div>
                      <div className="col-span-2 flex justify-end gap-1">
                        <button onClick={(e) => {
                            e.stopPropagation();
                            (0, generateEmployeePdf_1.generateEmployeePdf)(employee, selectedBusiness);
                        }} className="p-2 text-gray-400 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-lg transition-colors" title="Download PDF">
                          <lucide_react_1.Download className="w-5 h-5"/>
                        </button>
                        <button onClick={() => setSelectedEmployee(employee)} className="p-2 text-gray-400 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-slate-700 rounded-lg transition-colors">
...
function PeoplePage() {
    ...
    return (<react_1.Suspense fallback={<div className="p-8 text-center text-slate-500">Loading People...</div>}>
      {String((user === null || user === void 0 ? void 0 : user.role) || '').toUpperCase() === 'EMPLOYEE' ? <EmployeeSelfProfilePage_1.default /> : <AdminPeoplePageContent />}
    </react_1.Suspense>);
}
