...
const RoleMembersModal_1 = __importDefault(require("./RoleMembersModal"));
...
function RolesPage() {
    ...
    const fetchRoles = async () => {
        try {
            setLoading(true);
            const res = await api_1.default.get('/roles');
            setRoles(res.data);
        }
        catch (error) {
    ...
    const handleDelete = async (id) => {
        if (!confirm('Are you sure you want to delete this role?'))
            return;
        try {
            await api_1.default.delete(`/roles/${id}`);
            fetchRoles();
        }
        catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (selectedRole) {
                await api_1.default.patch(`/roles/${selectedRole.id}`, payload);
            }
            else {
                await api_1.default.post('/roles', payload);
            }
    ...
    return (<div className="flex flex-col h-full w-full bg-slate-50/50 dark:bg-slate-950/50">
      ...
      <div className="flex justify-between items-center bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-6 rounded-2xl border border-gray-200/50 dark:border-slate-700/50 shadow-sm">
        ...
        <button onClick={handleCreate} className="group relative px-6 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/40 transition-all duration-300 overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? ([1, 2, 3].map(i => <div key={i} className="h-48 bg-gray-100 dark:bg-slate-800 rounded-xl animate-pulse"/>)) : (roles.map(role => (<div key={role.id} className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 flex flex-col hover:border-indigo-500/50 transition-colors">
              <div className="flex justify-between items-start mb-3">
                ...
                {!role.isSystem && (<div className="flex gap-1">
                    <button onClick={() => handleManageMembers(role)} className="p-1.5 text-gray-400 hover:text-indigo-500 hover:bg-gray-50 dark:hover:bg-slate-700 rounded-lg transition-colors" title="Manage Members">
              ...
              <div className="mt-auto">
                ...
                var _a;
                return (<span key={p} className="px-2 py-1 bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 text-xs rounded-md">
                      {((_a = AVAILABLE_PERMISSIONS.find(ap => ap.id === p)) === null || _a === void 0 ? void 0 : _a.label) || p}
                    </span>);
            })}
                  {role.permissions.length > 3 && (<span className="px-2 py-1 bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-gray-400 text-xs rounded-md">
      ...
      <react_2.Transition show={isModalOpen} as={react_1.Fragment}>
        <react_2.Dialog as="div" className="relative z-50" onClose={() => setIsModalOpen(false)}>
          ...
          <div className="fixed inset-0 overflow-y-auto">
            <div className="flex min-h-full items-center justify-center p-4">
              <react_2.Transition.Child as={react_1.Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                <react_2.Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white dark:bg-slate-800 p-6 shadow-xl transition-all border border-gray-200 dark:border-slate-700">
                  <react_2.Dialog.Title className="text-lg font-bold text-gray-900 dark:text-white mb-6">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="text" value={name} onChange={e => setName(e.target.value)} required className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-indigo-500 dark:bg-slate-700 dark:text-white" placeholder="e.g. Shift Supervisor"/>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Description</label>
                        <input type="text" value={description} onChange={e => setDescription(e.target.value)} className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-indigo-500 dark:bg-slate-700 dark:text-white" placeholder="Brief description..."/>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-60 overflow-y-auto pr-2">
                        {AVAILABLE_PERMISSIONS.map(perm => (<div key={perm.id} onClick={() => togglePermission(perm.id)} className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-all ${permissions.includes(perm.id)
                ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-200 dark:border-indigo-800'
                : 'bg-gray-50 dark:bg-slate-900/50 border-transparent hover:bg-gray-100 dark:hover:bg-slate-700'}`}>
                            <div className={`mt-0.5 w-5 h-5 rounded border flex items-center justify-center transition-colors ${permissions.includes(perm.id)
                ? 'bg-indigo-600 border-indigo-600 text-white'
                : 'bg-white dark:bg-slate-800 border-gray-300 dark:border-slate-600'}`}>
                              {permissions.includes(perm.id) && <lucide_react_1.Check className="w-3 h-3"/>}
                            </div>
                            <div>
                              <div className={`text-sm font-medium ${permissions.includes(perm.id) ? 'text-indigo-900 dark:text-indigo-300' : 'text-gray-700 dark:text-gray-300'}`}>
                                {perm.label}
                              </div>
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 dark:border-slate-700">
                      <button type="button" onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-600">
                        Cancel
                      </button>
      ...
      <RoleMembersModal_1.default role={selectedRole} isOpen={isMembersModalOpen} onClose={() => setIsMembersModalOpen(false)} onUpdate={fetchRoles}/>
    </div>
  </div>);
}
