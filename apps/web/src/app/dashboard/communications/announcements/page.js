...
const AnnouncementCard_1 = __importDefault(require("@/components/announcements/AnnouncementCard"));
const CreateAnnouncementModal_1 = __importDefault(require("@/components/announcements/CreateAnnouncementModal"));
...
function AnnouncementsPage() {
    ...
    const [selectedAnnouncement, setSelectedAnnouncement] = (0, react_1.useState)(null);
    (0, react_1.useEffect)(() => {
        fetchAnnouncements();
    }, []);
    const fetchAnnouncements = async () => {
        try {
            setLoading(true);
            const res = await api_1.default.get('/announcements');
    ...
    const handleMarkRead = async (id) => {
        try {
            await api_1.default.post(`/announcements/${id}/read`);
    ...
    const handleDelete = async (id) => {
        if (!confirm('Are you sure you want to delete this announcement?'))
            return;
        try {
            await api_1.default.delete(`/announcements/${id}`);
    ...
    return (<div className="flex flex-col h-full w-full bg-slate-50/50 dark:bg-slate-950/50">
      ...
      <div className="flex flex-col sm:flex-row gap-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-4 rounded-2xl border border-gray-200/50 dark:border-slate-700/50 shadow-sm">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search announcements..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-gray-50 dark:bg-slate-800 border-none rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 transition-all"/>
        </div>
        <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0">
          {['ALL', 'NORMAL', 'HIGH', 'URGENT'].map((priority) => (<button key={priority} onClick={() => setFilterPriority(priority)} className={`px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${filterPriority === priority
      ...
      <div className="relative">
        ...
        <div className="space-y-6">
          <framer_motion_1.AnimatePresence>
            {loading ? (<div className="text-center py-12 text-gray-500">Loading announcements...</div>) : filteredAnnouncements.length === 0 ? (<div className="text-center py-12 bg-gray-50 dark:bg-slate-800/50 rounded-2xl border border-dashed border-gray-300 dark:border-slate-700">
                <lucide_react_1.Megaphone className="w-12 h-12 text-gray-400 mx-auto mb-3"/>
              ...
              </div>) : (filteredAnnouncements.map((announcement) => (<AnnouncementCard_1.default key={announcement.id} announcement={announcement} onMarkRead={handleMarkRead} onDelete={handleDelete} onEdit={handleEdit} canDelete={true} // In real app, check user role
        />)))}
          </framer_motion_1.AnimatePresence>
        </div>
      </div>

      <CreateAnnouncementModal_1.default isOpen={isModalOpen} onClose={() => {
            setIsModalOpen(false);
            setSelectedAnnouncement(null);
        }} onSuccess={fetchAnnouncements} initialData={selectedAnnouncement}/>
    </div>
  </div>);
}
