...
export const generatePayslip = async (payroll: any, business: any) => {
  const doc = new jsPDF();
  ...
  if (business?.logoUrl) {
    try {
      logoDataUrl = await getBase64ImageFromURL(business.logoUrl);
    } catch (error) {
  ...
  for (let i = 0; i < stubs.length; i++) {
    ...
    doc.setTextColor(0, 0, 0);
    doc.setFont('helvetica', 'normal');
    doc.text(format(new Date(payroll.payDate), 'MM/dd/yyyy'), gridX + colWidth/2, topGridY + rowHeight + 5, { align: 'center' });
    doc.text(payroll.type || 'Regular', gridX + colWidth*1.5, topGridY + rowHeight + 5, { align: 'center' });
    doc.text(format(new Date(payroll.periodEnd), 'MMM d'), gridX + colWidth*2.5, topGridY + rowHeight + 5, { align: 'center' });
  ...
  doc.save(`Payroll_${format(new Date(payroll.payDate), 'yyyy-MM-dd')}.pdf`);
};
